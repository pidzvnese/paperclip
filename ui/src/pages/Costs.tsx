...
function usePresetLabels(t: (key: string) => string): Record<DatePreset, string> {
  return {
    mtd: t("costs.monthToDate"),
    "7d": t("costs.last7Days"),
    "30d": t("costs.last30Days"),
    ytd: t("costs.yearToDate"),
    all: t("costs.allTime"),
    custom: t("costs.custom"),
  };
}
...
export function Costs() {
  ...
  const presetLabels = usePresetLabels(t);
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.costs") }]);
  }, [setBreadcrumbs, t]);
            ...
            <Card>
              <CardContent className="p-4">
                ...
                {data.byAgent.length === 0 ? (
                  <p className="text-sm text-muted-foreground">{t("costs.noCostEventsYet")}</p>
                ) : (
                      ...
                      >
                        ...
                        <div className="text-right shrink-0 ml-2 tabular-nums">
                          ...
                          <span className="text-xs text-muted-foreground block">
                            {t("costs.inOutToks", { in: formatTokens(row.inputTokens), out: formatTokens(row.outputTokens) })}
                          </span>
                          {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) && (
                            <span className="text-xs text-muted-foreground block">
                              {row.apiRunCount > 0 ? t("costs.apiRuns", { count: row.apiRunCount }) : null}
                              {row.apiRunCount > 0 && row.subscriptionRunCount > 0 ? " | " : null}
                              {row.subscriptionRunCount > 0
                                ? t("costs.subscriptionRuns", {
            ...
            <Card>
              <CardContent className="p-4">
                ...
                {data.byProject.length === 0 ? (
                  <p className="text-sm text-muted-foreground">{t("costs.noProjectAttributedCosts")}</p>
                ) : (
                  <div className="space-y-2">
